...
namespace debug {
    ...
    Mat readFeatureImage(string filename)
    {
        ...
        if (nChannels == 7) 
        {
            mat = Mat_<Vec<double,7> >(nRows,nCols);
        }
        else
        {
            mat = Mat_<Vec<double,17> >(nRows,nCols);
        }
    ...
    /** extract the content of a double image at the specified channel */
    Mat extractDoubleMat(Mat mat, int channel) {
        assert(channel >= 0 && channel < mat.channels());
        int nChannels = mat.channels();
        Mat submat = Mat::zeros(mat.rows,mat.cols,CV_64F);
        for (int r=0; r < mat.rows; r++) {
            double *ptr = (double *)mat.ptr<double>(r) + channel;
            double *subptr = (double *)submat.ptr<double>(r);
            for (int c=0; c < mat.cols; c++, ptr += nChannels, subptr++){
                *subptr = *ptr;
            }    
        }
        return submat;
